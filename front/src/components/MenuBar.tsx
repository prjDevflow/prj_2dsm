...
export default function MenuBar({ title }: { title?: string }) {
  ...
  // Verifica se está nas rotas onde os botões devem ser ocultados
  const shouldHideChartsAndTables = 
    location.pathname.startsWith("/furnas") || 
    location.pathname.startsWith("/balcar");
    ...
    <Nav>
      <TopContainer>
        ...
        <DesktopMenu>
          ...
          {/* Mostra TABELAS apenas se NÃO estiver em Furnas ou Balcar */}
          {!shouldHideChartsAndTables && (
            <MenuItem role="button" tabIndex={0} aria-label="Tabelas menu">
              <StyledLink to="/sima">TABELAS</StyledLink>
            </MenuItem>
          )}

          {/* Mostra GRÁFICOS apenas se NÃO estiver em Furnas ou Balcar */}
          {!shouldHideChartsAndTables && (
